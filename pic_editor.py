...
with col1:               # To display the header text using css style
    ...
    font-size:35px ; font-family: 'Cooper Black'; color: #FF6633;} 
    </style> """, unsafe_allow_html=True)
    st.markdown('<p class="font">Upload your photo here...</p>',
...
with st.sidebar.expander("About the App"):
    st.write("""
        Use this simple app to convert your favorite photo to a pencil sketch, a grayscale image or an image with blurring effect.  \n  \nThis app was created by Shahbaz Baig to learn Streamlit and computer vision. Hope you enjoy!
     """)
# Add file uploader to allow users to upload photos
uploaded_file = st.file_uploader("", type=['jpg', 'png', 'jpeg'])
filter = st.sidebar.radio('Covert your photo to:', [
                          'Original', 'Black and White', 'Pencil Sketch', 'Blur Effect', 'Crop'])  # Add the filter in the sidebar
...
if uploaded_file is not None:
    ...
    with col1:
        st.markdown('<p style="text-align: center;">Before</p>',
                    unsafe_allow_html=True)
        st.image(image, width=300)

    with col2:
        st.markdown('<p style="text-align: center;">After</p>',
                    unsafe_allow_html=True)
        ...
        if filter == 'Gray Image':
            ...
            slider = st.sidebar.slider(
                'Adjust the intensity', 1, 255, 127, step=1)
            (thresh, blackAndWhiteImage) = cv2.threshold(
                gray_scale, slider, 255, cv2.THRESH_BINARY)
            ...
            slider = st.sidebar.slider(
                'Adjust the intensity', 25, 255, 125, step=2)
            ...
            converted_img = np.array(image.convert('RGB'))
            slider = st.sidebar.slider(
                'Adjust the intensity', 5, 81, 33, step=2)
            converted_img = cv2.cvtColor(converted_img, cv2.COLOR_RGB2BGR)
            blur_image = cv2.GaussianBlur(
                converted_img, (slider, slider), 0, 0)
            st.image(blur_image, channels='BGR', width=300)
        elif filter == 'Crop':
            converted_img = np.array(image.convert('RGB'))
            cropped_image = converted_img[80:280, 150:330]
            st.image(cropped_image)
        else:
            st.image(image, width=300)

# Add a feedback section in the sidebar
# Used to create some space between the filter widget and the comments section
st.sidebar.title(' ')
# Used to create some space between the filter widget and the comments section
st.sidebar.markdown(' ')
